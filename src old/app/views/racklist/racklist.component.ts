...
@Component({
  selector: 'app-racklist',
  templateUrl: './racklist.component.html',
  styleUrls: ['./racklist.component.scss']
})
export class RacklistComponent implements OnInit {
  ...
  constructor(private userService: UserService, public router: Router,private toastr: ToastrService,private spinner: NgxSpinnerService,private titleService: Title) {
    this.titleService.setTitle("Rack List | Colorhunt");
  }
  ...
  public getRack() {
    this.userService.racklist().subscribe((res) => {
      const data = res;
  ...
  public delete(id) {
    Swal.fire({
      ...
      if (result.value == true) {
        this.userService.deleterack(id).subscribe((res) => {
          this.getRack();
          this.success(res);
  ...
  public edit(id) {
    this.router.navigate(['rack', { id: id }])
  }
