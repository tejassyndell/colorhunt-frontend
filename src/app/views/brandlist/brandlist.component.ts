...
export class BrandlistComponent implements OnInit {
  ...
  constructor(private userService: UserService, public router: Router, private toastr: ToastrService, private spinner: NgxSpinnerService, private http: HttpClient,private titleService: Title) {
    this.titleService.setTitle("Brand List | Colorhunt");
  }
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.http.post<DataTablesResponse>(
          this.ApiURL + "/brandpostlist",
          dataTablesParameters, {}
        ).subscribe(resp => {
          that.brandlist = resp.data;
      ...
      columns: [{ data: 'No' }, { data: 'Name' }, { data: 'Description' }, { data: 'Action' }]
    };
  }
  ...
  public delete(id) {
    Swal.fire({
      ...
      if (result.value == true) {
        this.userService.deletebrand(id).subscribe((res) => {
  ...
  public edit(id) {
    this.router.navigate(['brand', { id: id }])
  }

  success(data) {
    if (data.id != "") {
      this.toastr.success('Success', 'Brand Deleted Successfully');
    } else {
