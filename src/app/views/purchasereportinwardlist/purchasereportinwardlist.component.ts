...
@Component({
  selector: 'app-purchasereportinwardlist',
  templateUrl: './purchasereportinwardlist.component.html',
  styleUrls: ['./purchasereportinwardlist.component.scss']
})
export class PurchasereportinwardlistComponent  implements OnInit {
  ...
  constructor(private userService: UserService, public router: Router,private toastr: ToastrService,private spinner: NgxSpinnerService,private http: HttpClient,private titleService: Title) {
    this.titleService.setTitle("Purchase Inward Report | Colorhunt");
  }
      ...
      this.dtOptions = {
        ...
        ajax: (dataTablesParameters: any, callback) => {
          that.http.post<DataTablesResponse>(
            this.ApiURL+"/reportpostinwardlist",
  ...
  rightscheck(data, no) {
    ...
    if (no == 1) {
      ...
      for (let i = 0; i < Count; i++) {
        if (data[i].PageId == 3 ||  data[i].PageId == 7) {
