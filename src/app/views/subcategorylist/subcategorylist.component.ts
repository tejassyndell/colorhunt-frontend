...
export class SubcategorylistComponent implements OnInit {
  ...
  constructor(private userService: UserService, public router: Router, private toastr: ToastrService, private spinner: NgxSpinnerService,private http: HttpClient,private titleService: Title) {
    this.titleService.setTitle("Subcategory List | Colorhunt");
  }
      ...
      this.dtOptions = {
        ...
        ajax: (dataTablesParameters: any, callback) => {
          that.http.post<DataTablesResponse>(
              this.ApiURL+"/subcategorypostlist",
              dataTablesParameters, {}
            ).subscribe(resp => {
              that.subcategorylist = resp.data;
        ...
        columns: [{ data: 'No' }, { data: 'CategoryName' }, { data: 'Name' }, { data: 'Action' }]
      };
  }
  ...
  public delete(id) {
    Swal.fire({
      ...
      if (result.value == true) {
        this.userService.deletesubcategory(id).subscribe((res) => {
  ...
  success(data) {
    if (data.id != "") {
      this.toastr.success('Success', 'Sub category Deleted Successfully');
    } else {
